    ...
    module Taggable #:nodoc:
      ...
      module SingletonMethods
        ...
        def find_options_for_tag_counts(options = {})
          ...
          joins = ["INNER JOIN #{quoted_table_name} ON #{quoted_table_name}.#{primary_key} = #{Tagging.quoted_table_name}.taggable_id"]
          joins << options.delete(:joins) if options[:joins].present?
          #joins << relation[:joins] if relation && relation[:joins].present?
          joins = joins.join(" ")
